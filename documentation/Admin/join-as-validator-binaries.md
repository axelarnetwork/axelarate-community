...
1. Have an Axelar node fully caught up and running by completing the steps in `README.md`. Ensure you have some testnet coins on your validator address. Make sure you have the environment variables `CHAIN_ID` and `AXELARD_CHAIN_ID` set to `axelar-testnet-adelaide`.

```bash
export CHAIN_ID=axelar-testnet-adelaide
export AXELARD_CHAIN_ID=axelar-testnet-adelaide
```
...
Then go to [Axelar faucet](http://faucet.testnet.axelar.network/) and get some coins on your `broadcaster/proxy` address.  

Check that you received the funds:  

```bash
axelard q bank balances {proxy address} --home ~/.axelar_testnet/.vald
```
eg)
...
7. Check that you have some `axl` on your `broadcaster/proxy` address. Use [Axelar faucet](http://faucet.testnet.axelar.network/) to get some coins.
